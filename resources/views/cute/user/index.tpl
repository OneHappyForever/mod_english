							...
							<div class="card">
								<div class="card-main">
									<div class="card-inner">
										...
										<dl class="dl-horizontal">
											<dd>帐号等级 <code>VIP{$user->class}</code></dd>
											<dd>等级过期时间：{$user->class_expire}</dd>
											<dd>帐号过期时间：{$user->expire_in}</dd>
											{if $user->node_speedlimit!=0}
											<dd>速度限制：{$user->node_speedlimit}Mbps</dd>
											{else}
											<dd>速度限制：不限速</dd>
											{/if}
											<dd>上次使用：{$user->lastSsTime()}</dd>
											<dd>
												<p>已用 {$user->LastusedTraffic()}</p>
												<p>今日 {$user->TodayusedTraffic()}</p>
												<p>剩余 {$user->unusedTraffic()}</p>
											</dd>											
										</dl>
									</div>
